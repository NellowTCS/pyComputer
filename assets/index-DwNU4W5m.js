...
`},cs=`modulepreload`,ls=function(e,t){return new URL(e,t).href},us={},ds=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}function s(e){return import.meta.resolve?import.meta.resolve(e):new URL(e,import.meta.url).href}r=o(t.map(t=>{if(t=ls(t,n),t=s(t),t in us)return;us[t]=!0;let r=t.endsWith(`.css`);for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}let i=document.createElement(`link`);if(i.rel=r?`stylesheet`:cs,r||(i.as=`script`),i.crossOrigin=``,i.href=t,a&&i.setAttribute(`nonce`,a),document.head.appendChild(i),r)return new Promise((e,n)=>{i.addEventListener(`load`,e),i.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})};async function fs(){let{loadPyodide:e}=await ds(async()=>{let{loadPyodide:e}=await import(`https://cdn.jsdelivr.net/pyodide/v0.29.3/full/pyodide.mjs`);return{loadPyodide:e}},[],import.meta.url);return e({indexURL:`https://cdn.jsdelivr.net/pyodide/v0.29.3/full/`})}async function ps(e,t){await e.loadPackage(`micropip`),await e.runPythonAsync(`import micropip`);let n=JSON.stringify(t);await e.runPythonAsync(`import micropip\nawait micropip.install(${n})`)}var ms=document.getElementById(`status`),hs=document.getElementById(`terminal`),gs={default:{bg:`#0a0a0a`,fg:`#ffffff`,cursor:`#ffffff`,selection:`rgba(255, 255, 255, 0.3)`},retro:{bg:`#0a0a0a`,fg:`#00ff41`,cursor:`#00ff41`,selection:`rgba(0, 255, 65, 0.3)`},light:{bg:`#ffffff`,fg:`#000000`,cursor:`#000000`,selection:`rgba(0, 0, 0, 0.2)`},dark:{bg:`#0a0a0a`,fg:`#ffffff`,cursor:`#ffffff`,selection:`rgba(255, 255, 255, 0.3)`},mono:{bg:`#0a0a0a`,fg:`#cccccc`,cursor:`#cccccc`,selection:`rgba(255, 255, 255, 0.2)`},pastel:{bg:`#1e1e2e`,fg:`#cba6f7`,cursor:`#cba6f7`,selection:`rgba(203, 166, 247, 0.3)`}};function _s(e){let t=gs[e]||gs.default;document.body.style.background=t.bg,document.body.style.color=t.fg,document.documentElement.style.setProperty(`--term-bg`,t.bg);let n=document.getElementById(`terminal-container`);n&&(n.style.background=t.bg),window.term&&(window.term.options.theme={background:t.bg,foreground:t.fg,cursor:t.cursor,selection:t.selection}),document.querySelectorAll(`#theme-buttons button`).forEach(e=>e.classList.remove(`active`));let r=document.querySelector(`#theme-buttons button[data-theme="${e}"]`);r&&r.classList.add(`active`)}window.setWebTheme=_s;var $=new rs({cursorBlink:!0,cursorStyle:`block`,fontSize:14,fontFamily:`'Fira Code', 'Consolas', monospace`,theme:{background:`#0a0a0a`,foreground:`#ffffff`,cursor:`#ffffff`,selection:`rgba(255, 255, 255, 0.3)`}});window.term=$;var vs=new os;$.loadAddon(vs),$.open(hs),vs.fit(),window.addEventListener(`resize`,()=>vs.fit()),window.pythonLineBuffer=[],window.termWrite=e=>{let t=e.replace(/\r\n/g,`
`).replace(/\r/g,`
`).replace(/\n/g,`\r
`);$.write(t)};var ys=!1,bs=null;function xs(){if(!bs)try{bs=Ss.pyimport(`pycomputer.ui.input`)}catch{}return bs}window.setRawInput=e=>{ys=e};var Ss=null,Cs=[],ws=``,Ts=null;window.pyc_readline=()=>new Promise(e=>{Cs.length>0?e(Cs.shift()):Ts=e});function Es(e){Ts?(Ts(e),Ts=null):Cs.push(e)}function Ds(){return Cs.length>0?Promise.resolve(Cs.shift()):new Promise(e=>{Ts=e})}$.onData(e=>{if(ys){let t=xs();if(t)try{if(e.startsWith(`\x1B`))t.web_input_queue.append(e);else for(let n of e)t.web_input_queue.append(n)}catch{}return}for(let t of e)t===`\r`?($.write(`\r
`),Es(ws),ws=``):t===``?ws.length>0&&(ws=ws.slice(0,-1),$.write(`\b \b`)):t===``?($.write(`^C\r
`),Es(`exit`),ws=``):(ws+=t,$.write(t))}),$.write(`\x1B[2J\x1B[1;1HLoading pyComputer...
`);async function Os(){Ss=await fs(),ms.textContent=`Ready`,ms.className=`ready`,$.write(`\r
`);try{await ps(Ss,[`tuiro`,`requests`]),$.write(`[web] Installed web dependencies: tuiro, requests\r
...
kernel.boot_sequence()
shell = kernel.shell
`;await Ss.runPythonAsync(e);let t=`[/] $ `;$.write(t);async function n(e){if(!e.trim()){$.write(t);return}Ss.globals.set(`commandLine`,e);try{await Ss.runPythonAsync(`shell.execute(commandLine)`)}catch(e){if(String(e).includes(`SystemExit`)){$.write(`Shell exited.\r
`);return}$.write(`Error: ${e}\r\n`)}$.write(t)}for(;;)await n(await Ds())}Os().catch(e=>{ms.textContent=`Failed`,$.write(`Error: ${e}\r\n`)});