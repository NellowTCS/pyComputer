...
`);try{await zs(Ks,[`tuiro`,`requests`]),$.write(`[web] Installed web dependencies: tuiro, requests\r
...
kernel.boot_sequence()
shell = kernel.shell
`;await Ks.runPythonAsync(e);let t=`[/] $ `;$.write(t);async function n(e){if(!e.trim()){$.write(t);return}Ks.globals.set(`commandLine`,e);try{await Ks.runPythonAsync(`shell.execute(commandLine)`)}catch(e){if(String(e).includes(`SystemExit`)){$.write(`Shell exited.\r
`);return}$.write(`Error: ${e}\r\n`)}$.write(t)}for(;;)await n(await Zs())}Qs().catch(e=>{Bs.textContent=`Failed`,$.write(`Error: ${e}\r\n`)});